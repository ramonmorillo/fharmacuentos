  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-10">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-brand-900">Biblioteca de plantillas</h1>
          <p className="text-brand-700/80 text-sm mt-1">
        ...
        <button onClick={onBack} className="no-print text-sm text-brand-600 hover:underline whitespace-nowrap">
          ← Volver
        </button>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        {STORY_TEMPLATES.map((template) => (
          <div
            key={template.id}
            className="bg-white rounded-xl border border-brand-100 p-4 shadow-sm flex flex-col justify-between"
          >
            <div>
              <h2 className="font-semibold text-brand-800">{template.title}</h2>
              <p className="text-sm text-brand-700/80 mt-1">{template.description}</p>
            ...
            <button
              ...
              className="mt-4 self-start px-4 py-2 rounded-lg bg-brand-600 text-white text-sm font-medium hover:bg-brand-700 transition-colors"
            >
              Usar esta plantilla
            </button>
          </div>
